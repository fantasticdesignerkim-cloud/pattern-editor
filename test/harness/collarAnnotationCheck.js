...
function draft(withAnchors) {
  ...
  const cd = { sourceBodiceHash: "BH1", type: "shirt-two-piece", baseMethod: "bunka-shirt-collar-M-v2",
    parameters: { stand: { standHeightCm: RP.standHeightCm, frontRiseCm: RP.frontRiseCm } }, standGeometry: st.standGeometry, collarGeometry: null,
...
// 1. M 기본형 표시 모델 — 입력값은 parameters 그대로, 보조선은 named anchors 그대로
{
  ...
  ok(m && m.mode === "parametric" && m.note === null && m.recipe === "bunka-shirt-collar-M-v2", "1: parametric 모델");
  const inp = {}; m.inputs.forEach(r => { inp[r.key] = r.value; });
  ok(inp.standHeightCm === 3 && inp.frontRiseCm === 1 && inp.gapCm === 3 && inp.cbWidthCm === 4 && inp.frontInsetCm === 0.5 &&
  ...
  ok(D["band-width"].text === 3 && D["cf-rise"].text === 1 && D.gap.text === 3 && D["cb-width"].text === 4 && D.setback.text === 0.5 &&
  ...
  ok(same(D.setback.from, bd.anchors.bandTopCf) && same(D.setback.to, bd.anchors.setbackPoint), "1: setback = 657b2e5 bandTopCf→setbackPoint anchor(재발명 없음)");
  ...
  ok(same(D["point-diagonal"].from, bd.anchors.setbackPoint) && same(D["point-diagonal"].to, bd.anchors.tip) &&
     same(D.projection.to, bd.anchors.tip) && D.projection.from.x === bd.anchors.setbackPoint.x && D.projection.from.y === bd.anchors.tip.y, "1: 사선·수평 돌출 = setbackPoint·tip anchor");
  ok(same(bd.anchors.setbackPoint, DC.computeBody(DC.computeStand(BODICE, RP), RB).anchors.setbackPoint), "1: 표시 anchor = 엔진 재계산 anchor(결정론)");
  ok(m.labels.map(l => l.id).join(",") === "cb,cf,setback", "1: CB/CF/setback 라벨");
  ...
  ok(R.bandAttach.value === bm.bandAttachLenCm && R.upperSeam.value === bm.upperCollarSeamLenCm && R.seamDiff.value === bm.seamLengthDiffCm && R.seamDiff.status === "match", "1: 이음선·길이차 = body measure · 정합");
  ...
  ok(R.actualCbWidth.value === bm.cbWidthCm && R.actualProjection.value === bm.frontProjectionCm && R.actualDiagonal.value === bm.pointDiagonalLenCm &&
     Math.abs(R.actualSetback.value - 0.5) < 1e-9, "1: 실제 CB 폭·setback·돌출·사선 = measures");
...
// 3. manual: 본체 보조선 숨김·안내, 스탠드 보조선 유지, 부재 measure 는 null
{
  ...
  ok(ids.indexOf("band-width") >= 0 && ["gap", "cb-width", "setback", "projection", "point-diagonal"].every(i => ids.indexOf(i) < 0) && m.labels.every(l => l.id !== "setback"), "3: manual → 본체 보조선 숨김(스탠드만)");
  const R = {}; m.results.forEach(x => { R[x.key] = x; });
  ok(R.bandAttach.value === null && R.seamDiff.value === null && R.seamDiff.status === null && R.cbCorrection.value === null && R.actualDiagonal.value === r.measure.pointDiagonalLenCm, "3: 부재 measure 는 null(재계산 없음)");
}
