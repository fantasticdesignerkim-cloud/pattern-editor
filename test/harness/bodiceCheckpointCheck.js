...
// ══════════════════════════════════════════════
{
  ...
  {
    const r = sem(mkProject({ schemaVersion: 7 }));
    ok(r.ready === true && r.issues.length === 0, "11a: untouched v7 → ready, issues 없음");
    ok(r.sourceSchemaVersion === 7 && r.unresolved.length === 0 && r.missing.length === 0, "11a: 목록 비어있음");
  }
  ...
  {
    const base = mkProject({ schemaVersion: 7 });
  ...
  {
    const clean = mkProject({ schemaVersion: 7 });
    PROJECT = clean; const r1 = BC.complete();
    const withFlag = mkProject({ schemaVersion: 7 });
...
// ══════════════════════════════════════════════
{
  ...
  }
  {
    ...
    ok(r.ready === true && r.issues.length === 0, "12b: v7 + 완전 다트 → ready");
  }
  ...
  // (e) manual replacement 가 다트 target boundary 를 끊으면 complete 로 두지 않는다
  {
    ...
    const base = mk(7, darts, { refs: false });   // 기준 경계 fixture 없이 — 경계 끊김만 본다
  ...
  // (g) semantic fingerprint — 의미만 바뀌어도 바뀌고, 기존 형상 hash 는 불변
  {
    ...
    // 같은 의미·같은 좌표면 동일
    const p3 = mk(7, d1); ok(BC.evaluateSemantics(p3).fingerprint === r1.result.semantics.fingerprint, "12g: 동일 의미 → 동일 fingerprint");
  }

  // (h) 다트 의미가 좌표·kind·개수·순서를 바꾸지 않는다
  {
    const bare = mk(7, [line([40, 20], [44, 38]), line([48, 20], [44, 38])], { refs: false });
    const tagged = mk(7, [dleg([40, 20], [44, 38], "front-waist-a", "armhole", "to"),
