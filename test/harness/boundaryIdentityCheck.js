...
// ── bodiceCheckpoint: 증거·ordered chain·fingerprint(형상 hash 분리) ──
{
  ...
  const base = mk(8, true); PROJECT = base; const c1 = BC.complete();
  const dirP = mk(8, true); dirP.working.geometry.front.outline[3].boundary.ranges = [[1, 0]];
  PROJECT = dirP; const c2 = BC.complete();
  const rootP = mk(8, true); rootP.working.geometry.back.outline[4].boundary.root = "back/armhole-upper";
  PROJECT = rootP; const c3 = BC.complete();
  const rangeP = mk(8, true); rangeP.working.geometry.front.outline[1].boundary.ranges = [[0, 0.5]];
  ...
  // 배치·UI 상태 제외
  const lay = mk(8, true); lay.working.layout = { front: { dx: 9, dy: 9 } }; lay.working.selectedId = "line-1";
  ...
  const mis = mk(8, true); mis.working.geometry.front.outline[4].boundary.root = "front/shoulder";   // armhole prim 에 shoulder root
  const rm = sem(mis);
  ...
  const cnt = mk(8, true); cnt.working.geometry.front.outline[2].boundary.ranges = [[0, 0.5], [0.5, 1]];
  ok(sem(cnt).issues.indexOf("boundary-identity-misaligned") >= 0, "8: 구간 수 ≠ 명령 수 → misaligned");
  const miss = mk(8, true); delete miss.working.geometry.back.outline[0].boundary;
  const rmi = sem(miss);
  ok(!rmi.ready && rmi.issues.indexOf("boundary-identity-missing") >= 0 && rmi.boundaries.missing.some(m => m.piece === "back" && m.role === "center"), "8: v8 필수 role identity 누락 → missing");
  ...
  const ro = mk(8, true); const fo = ro.working.geometry.front.outline; [fo[0], fo[1]] = [fo[1], fo[0]];
  const rro = sem(ro);
  ...
  const rb2 = mk(8, true); rb2.working.geometry.back.outline.reverse();
  ...
  const oob = mk(8, true); oob.working.geometry.back.outline[1].boundary.ranges = [[0, 1.01]];
  const rob = sem(oob);
  ...
  const neg = mk(8, true); neg.working.geometry.front.outline[0].boundary.ranges = [[-0.001, 1]];
  ok(sem(neg).issues.indexOf("boundary-identity-misaligned") >= 0, "8: 범위 밖(-0.001) → misaligned");
  const epsOk = mk(8, true); epsOk.working.geometry.front.outline[0].boundary.ranges = [[0, 1 + 5e-7]];
