...
// ── bodiceCheckpoint: 증거·ordered chain·fingerprint(형상 hash 분리) ──
{
  ...
  const r5 = sem(mk(5, true));
  ok(r5.ready === true && r5.boundaries.missing.length === 0 && r5.boundaries.misaligned.length === 0, "8: v5 + 전부 선언 → ready");
  ok(r5.boundaries.front.length === 5 && r5.boundaries.front[0].root === "front/center", "8: effective outline 순서의 span reference 증거");
  ok(typeof r5.boundaryFingerprint === "string" && r5.boundaryFingerprint === sem(mk(5, true)).boundaryFingerprint, "8: fingerprint 결정론");

  // 의미만 변경 → boundaryFingerprint 변화, 형상 hash·측정·dart fingerprint 불변
  const base = mk(5, true); PROJECT = base; const c1 = BC.complete();
  const dirP = mk(5, true); dirP.working.geometry.front.outline[3].boundary.ranges = [[1, 0]];
  PROJECT = dirP; const c2 = BC.complete();
  const rootP = mk(5, true); rootP.working.geometry.back.outline[4].boundary.root = "back/armhole-upper";
  PROJECT = rootP; const c3 = BC.complete();
  const rangeP = mk(5, true); rangeP.working.geometry.front.outline[1].boundary.ranges = [[0, 0.5]];
  ...
  // 배치·UI 상태 제외
  const lay = mk(5, true); lay.working.layout = { front: { dx: 9, dy: 9 } }; lay.working.selectedId = "line-1";
  ...
  const mis = mk(5, true); mis.working.geometry.front.outline[4].boundary.root = "front/shoulder";   // armhole prim 에 shoulder root
  const rm = sem(mis);
  ...
  const cnt = mk(5, true); cnt.working.geometry.front.outline[2].boundary.ranges = [[0, 0.5], [0.5, 1]];
  ok(sem(cnt).issues.indexOf("boundary-identity-misaligned") >= 0, "8: 구간 수 ≠ 명령 수 → misaligned");
  const miss = mk(5, true); delete miss.working.geometry.back.outline[0].boundary;
  const rmi = sem(miss);
  ok(!rmi.ready && rmi.issues.indexOf("boundary-identity-missing") >= 0 && rmi.boundaries.missing.some(m => m.piece === "back" && m.role === "center"), "8: v5 필수 role identity 누락 → missing");
