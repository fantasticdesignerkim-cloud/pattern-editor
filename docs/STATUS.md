...
1. **`161 닫는다·벌린다` 를 디자인 단계에 도입** → 플레어 G·H + 목둘레 턱 I·J 네 개가 한꺼번에 풀린다.
   절개선은 **다트 apex → 밑단**으로 확정됐다(P.20 판독). 구현 경로 두 갈래를 아직 안 골랐다:
   **(A)** `designLineTool` 의 검증된 ring/split 코드를 확장 — `splitRingByCut` 이 "끝점이 다트 다리
   위"인 절개선을 거부하는 한 곳만 열면 된다. **(B)** `dartMove` 엔진을 어댑터로 재사용.
   → 권고는 (A)(원형 골든 위험 0). **강체 회전이라 면적이 정확히 보존**되는 게 강한 불변식이다.
