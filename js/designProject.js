...
//  - schemaVersion 수용: **5 = 정상**(봉제 경계 의미 + 다트 의미 + 경계 안정 identity),
//    **2·3·4 = legacy 수용**(형상 입력으로는 쓰되 `semanticStatus:"legacy-incomplete"` 로 표시 —
...
(function () {
  ...
  function startFromBlock(completed) {
    ...
    if (sv !== 2 && sv !== 3 && sv !== 4 && sv !== 5) fail("unsupported-schema-version", sv);
    // v2 = 신규 봉제 경계 의미가 없는 구형 완료본. 형상은 쓰되 legacy 로 표시하고
    // role 을 지어내지 않는다(좌표 휴리스틱 재태깅 금지).
    // v5 = 봉제 경계 의미 + 구조화 다트 의미 + 경계 안정 identity(P0.3a)를 모두 실은 정상 완료본.
    // v2/v3/v4 = 누락 의미가 있는 legacy — 없는 의미(경계 identity 포함)를 만들어 넣지 않는다.
    const semanticStatus = (sv === 5) ? "complete" : "legacy-incomplete";
    if (_project) {
