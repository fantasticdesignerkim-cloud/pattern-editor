...
(function () {
  ...
  // 현재 형상의 봉제 허리다트 합(목표 허리로 역산했을 때 "무엇이 나왔는지" 보여 주기 위해).
  function sewnDartTotal(project) {
    const g = project && project.working && project.working.geometry;
    if (!g || !window.designBodice || typeof window.designBodice.sewnWaistDartTotal !== "function") return null;
  ...
  function bodyStatusNote(E, L, W, H, Cv, neckType, Dt, Wt, DtActual) {
    ...
    // 목표 허리는 역산 결과(실제 다트량)를 함께 보여 준다 — 무엇으로 달성했는지 감추지 않는다.
    if (Wt != null) parts.push("목표 허리 " + fmtL(Wt) + "cm" + (DtActual != null ? "(다트 " + fmtL(DtActual) + "cm)" : ""));
  ...
  function readBodyInputs() {
    ...
    // 목표 완성 허리: 빈 값 = 미사용. 다트량 직접 지정과 **동시 사용 금지**(과결정).
    const tgtEl = document.getElementById("inpBodyWaistTarget");
    const tgtRaw = tgtEl ? String(tgtEl.value).trim() : "";
    const tgtNum = tgtRaw === "" ? null : Number(tgtRaw);
    const target = { input: tgtEl, v: tgtNum, valid: tgtRaw === "" || (isFinite(tgtNum) && tgtNum >= 30 && tgtNum <= 200) };
    const bothWaist = (dartNum != null && tgtNum != null);
    ...
    return {
      ease, len, waist, hem, curve, dart, target, bothWaist, neckType, nW, nF, nB, nCA, nVD, nSW, nCR,
      valid: ease.valid && len.valid && waist.valid && hem.valid && curve.valid && dart.valid && target.valid && !bothWaist &&
  ...
  function isShawlCollar(project) { return activeCollarKind(project) === "shawl"; }
  ...
  function refreshCollarStale(project) {
    ...
    if (cd && collarStale(project)) {
      ...
      cd.standGeometry = null; cd.body = null; cd.standalone = null; cd.tip = null; cd.joined = null; cd.flat = null; cd.sailor = null; cd.bow = null; cd.frill = null; cd.hood = null; cd.tailored = null; cd.shawl = null;   // 파생 카라 형상을 한 벌로 숨김
    }
  }
  ...
  // 숄 칼라(j·k): 수치 입력 없이 교재 기본값으로 적용한다. 깃아귀가 없어 외곽이 한 줄기다.
  function shawlMeasure(project) {
    const cd = project && project.working && project.working.collarDraft;
    return (cd && cd.type === "shawl-collar" && cd.shawl) ? (cd.shawl.measure || null) : null;
  }
  function shawlSummary(m) {
    return "뒤 달림선 " + fmtL(m.backAttachLenCm) + "cm = 뒤 목둘레 " + fmtL(m.backNeckLenCm) + "cm(누임 " +
      fmtL(m.layDownCm) + "cm · " + fmtL(m.layDownAngleDeg) + "°) · 칼라 허리 " + fmtL(m.cbStandLenCm) + "·폭 " +
      fmtL(m.cbWidthLenCm) + "cm · 라펠 폭 " + fmtL(m.lapelWidthLenCm) + "cm · 외곽 한 줄기 " +
      fmtL(m.outerLenCm) + "cm(칼라 " + fmtL(m.collarOuterLenCm) + " + 라펠 " + fmtL(m.lapelOuterLenCm) +
      ") · 깃아귀 없음 · 안단 별도 · 세션 전용";
  }
  function updateShawlCollarPanel(project, gate, pd) {
    const applyBtn = document.getElementById("btnApplyCollar"), resetBtn = document.getElementById("btnResetCollar");
    const m = shawlMeasure(project), stale = collarStale(project);
    syncCollarPresetLabel();
    if (applyBtn) applyBtn.disabled = !gate || !pd.ok;
    if (resetBtn) resetBtn.disabled = !(project.working.collarDraft);
    if (!gate) setCollarNote("소매 완료 후 카라를 편집할 수 있습니다");
    else if (!pd.ok) setCollarNote(collarSelectionStr(pd.reason) + (m ? " · 현재 카라 형상은 그대로 유지" : ""));
    else if (m && stale) setCollarNote("몸판 변경됨 · 숄 칼라 다시 적용 필요 · 세션 전용");
    else if (m) setCollarNote(shawlSummary(m));
    else setCollarNote("앞 몸판 위에 라펠 → 위 칼라 순으로 제도합니다(깃아귀 없음 · 안단은 별도) · 세션 전용");
    syncCollarBodyModeUI(project);
    updateCollarCheckpointUI(project);
    updateCollarDraftSummary(project);
  }
  ...
  function updateCollarPanel(project) {
    ...
    if (isShawlCollar(project)) { updateShawlCollarPanel(project, gate, pd); return; }
  ...
  function collarCPFailStr(reason) {
    const m = { "no-bodice": "몸판 완료 필요", "bodice-stale": "몸판 변경됨 · 다시 완료 필요", "no-collar": "카라 적용 필요",
      ...
      "no-shawl-collar": "숄 칼라 적용 필요", "shawl-recompute": "숄 칼라 재계산 실패",
      "outer-not-continuous": "외곽선이 한 줄기가 아님(칼라+라펠 합이 전체와 다름)",
      "shawl-has-notch": "숄 칼라에 깃아귀 수치가 섞였습니다",
      "outer-curve-failed": "칼라 외곽 곡선을 만들 수 없습니다", "attach-curve-failed": "칼라 달림선 곡선을 만들 수 없습니다",
    ...
    if (reason && reason.indexOf("shawl-") === 0) return "숄 칼라 형상 오류(" + reason.slice(6) + ")";
  ...
  function updateCollarCheckpointUI(project) {
    ...
    else if (checkNote && cd && cd.type === "shawl-collar") {
      const m = (cd.shawl && cd.shawl.measure) || null;
      const sym = collarDraftSymbol(cd);
      checkNote.textContent = m ? "숄 칼라" + (sym ? " " + sym : "") + ": 뒤 달림선 " + fmtL(m.backAttachLenCm) + "·뒤 목둘레 " + fmtL(m.backNeckLenCm) + "cm(누임 " + fmtL(m.layDownAngleDeg) + "°) · 칼라 허리 " + fmtL(m.cbStandLenCm) + "·폭 " + fmtL(m.cbWidthLenCm) + "·라펠 " + fmtL(m.lapelWidthLenCm) + "cm · 외곽 " + fmtL(m.outerLenCm) + "cm(칼라 " + fmtL(m.collarOuterLenCm) + ") · 이음선 " + fmtL(m.seamLenCm) + "cm" : "";
    }
  ...
  function updateDesignBodyPanel() {
    ...
    { const el = document.getElementById("inpBodyWaistTarget");
      if (el && document.activeElement !== el) el.value = (cb.Wt == null) ? "" : fmtL(cb.Wt); }
  ...
  function onApplyBodyLength() {
    ...
    if (st.bothWaist) { setBodyNote("목표 완성 허리와 허리 다트량은 함께 쓸 수 없습니다 · 하나만 채우세요"); syncBodyButtons(); return; }
    if (!st.valid) { setBodyNote("입력값 범위를 확인하세요(여유량·길이 0–100, 옆선 −30–30, 목표 허리 30–200)"); syncBodyButtons(); return; }
    const E = st.ease.v, L = st.len.v, W = st.waist.v, H = st.hem.v, Cv = st.curve.v;
    const Dt = st.dart.v;   // null = 원형 다트 그대로
    const Wt = st.target.v; // null = 목표 허리 미사용
