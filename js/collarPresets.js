...
(function () {
  ...
  // 플랫 칼라 S(교재 P.69) 파라미터. 키 순서 = designCollar.computeFlatCollarS 계약.
  //   달림선은 **몸판 목둘레선 그대로**라 입력이 없고(파생), 어깨 맞댐도 수치가 아니다.
  var FLAT_COLLAR_FIELDS = [
    { key: "collarWidthCm", label: "칼라 폭(뒤 중심·어깨)", unit: "cm", min: 0, minExclusive: true },
    { key: "frontEndFromFnpCm", label: "앞 칼라 폭(FNP→칼라 끝)", unit: "cm", min: 0, minExclusive: true },
    { key: "frontEndOffsetCm", label: "칼라 끝 안내선(앞 중심선에서)", unit: "cm", min: 0 }
  ];
  // 플랫 칼라 T(교재 P.69 하단) 파라미터. 키 순서 = designCollar.computeFlatCollarT 계약.
  //   S 와 달리 어깨선을 겹치고(3.5) 뒤 중심 달림선을 0.5 올려 재작도한다.
  var FLAT_COLLAR_T_FIELDS = [
    { key: "collarWidthCm", label: "칼라 폭(뒤 중심·어깨)", unit: "cm", min: 0, minExclusive: true },
    { key: "cbRiseCm", label: "뒤 중심 달림선 올림", unit: "cm", min: 0 },
    { key: "shoulderOverlapCm", label: "어깨선 겹침(어깨 끝 간격)", unit: "cm", min: 0 },
    { key: "frontEndFromFnpCm", label: "앞 칼라 폭(FNP→칼라 끝)", unit: "cm", min: 0, minExclusive: true },
    { key: "frontEndOffsetCm", label: "칼라 끝 안내선(앞 중심선에서)", unit: "cm", min: 0 }
  ];
  // 세일러 칼라(교재 U·V·W, P.70 · 제도 방법 P.150) 파라미터. 키 순서 = designCollar.computeSailorCollarU 계약.
  //   U·V·W 는 **같은 제도(P.150)** 이고 수치만 다르다 — 생성기·필드·키 순서를 공유한다.
  //   V 목둘레(FNP 에서 내림 · 현에서 휨)는 **칼라 안에서만 파생**한다 — 몸판 geometry 는 바꾸지 않는다.
  var SAILOR_COLLAR_FIELDS = [
    { key: "vDropCm", label: "V 목둘레(FNP에서 내림)", unit: "cm", min: 0, minExclusive: true },
    { key: "vHollowCm", label: "V선 휨(현에서)", unit: "cm", min: 0 },
    { key: "shoulderOverlapCm", label: "어깨선 겹침", unit: "cm", min: 0 },
    { key: "cbRiseCm", label: "뒤 중심 달림선 올림", unit: "cm", min: 0 },
    { key: "cbWidthCm", label: "뒤 중심 칼라 폭", unit: "cm", min: 0, minExclusive: true },
    { key: "backOuterCm", label: "뒤 칼라 외곽(뒤 중심에 직각)", unit: "cm", min: 0, minExclusive: true },
    { key: "shoulderWidthCm", label: "어깨 칼라 폭", unit: "cm", min: 0, minExclusive: true },
    { key: "frontOuterBowCm", label: "앞 외곽선 휨(현에서)", unit: "cm", min: 0 }
  ];
  // 보 칼라(교재 X·Y·Z, P.71) 파라미터. 키 순서 = designCollar.computeBowCollar 계약.
  //   X·Y·Z 는 **같은 제도**(교재: Y·Z 는 "Ⓧ와 같이 제도한다")이고 칼라 폭·리본 길이만 다르다 —
  //   생성기·필드·키 순서를 공유한다. 칼라 달림 끝 3 은 세 도해 공통 표기다.
  var BOW_COLLAR_FIELDS = [
    { key: "collarWidthCm", label: "칼라 폭", unit: "cm", min: 0, minExclusive: true },
  ...
  // 테일러드 칼라(교재 h·i, P.78·79 · 제도 방법 P.152–153) 파라미터. 키 순서 = designCollar.computeTailoredCollar 계약.
  //   ★ 교재 9교시 머리말: "칼라 폭은 일정하게 두고, **칼라 허리의 높이와 누임 치수를 변경한 2종류**".
  //     i 본문이 인과까지 밝힌다 — 허리를 낮추면 칼라 외곽 치수가 부족해져 누임을 늘린다(3/2.5 ↔ 1/6).
  //   SNP 에서 어깨선 연장 0.7 은 h·i 공통이라 **제도법 상수**(designCollar.TAILORED_METHOD)로 두고
  //     레코드에 복제하지 않는다.
  var TAILORED_FIELDS = [
    { key: "collarStandCm", label: "칼라 허리", unit: "cm", min: 0, minExclusive: true },
    { key: "collarWidthCm", label: "칼라 폭(뒤)", unit: "cm", min: 0, minExclusive: true },
    { key: "lapelWidthCm", label: "라펠 폭", unit: "cm", min: 0, minExclusive: true },
    { key: "layDownCm", label: "누임 치수(★)", unit: "cm", min: 0, minExclusive: true },
    { key: "neckShiftCm", label: "목둘레 이동량", unit: "cm", min: 0 },
    { key: "tipRadiusCm", label: "칼라 끝(반원 반지름)", unit: "cm", min: 0, minExclusive: true },
    { key: "lapelBowCm", label: "라펠 외형 휨", unit: "cm", min: 0 },
    { key: "frontNeckCm", label: "앞 목둘레선(깃아귀 방향)", unit: "cm", min: 0 },
    { key: "frontRiseCm", label: "앞 중심 올림", unit: "cm", min: 0 }
  ];
  // 숄 칼라(교재 j·k, P.80·81 · 제도 방법 P.154–155) 파라미터. 키 순서 = designCollar.computeShawlCollar 계약.
  //   테일러드와 단계 1~4 를 공유하지만 **깃아귀가 없어** tipRadiusCm·lapelBowCm 이 없다 —
  //   외곽이 뒤 중심에서 꺾임 끝까지 한 줄기라 칼라 끝 작도도 라펠 외형 휨도 쓰지 않는다.
  var SHAWL_FIELDS = [
    { key: "collarStandCm", label: "칼라 허리", unit: "cm", min: 0, minExclusive: true },
    { key: "collarWidthCm", label: "칼라 폭(뒤)", unit: "cm", min: 0, minExclusive: true },
    { key: "lapelWidthCm", label: "라펠 폭", unit: "cm", min: 0, minExclusive: true },
    { key: "layDownCm", label: "누임 치수(★)", unit: "cm", min: 0, minExclusive: true },
    { key: "neckShiftCm", label: "목둘레 이동량", unit: "cm", min: 0 },
    { key: "frontNeckCm", label: "앞 목둘레선(이음선 기준)", unit: "cm", min: 0 },
    { key: "frontRiseCm", label: "앞 중심 올림", unit: "cm", min: 0 }
  ];
  ...
  var CATALOG = [
    ...
    // 플랫 칼라 family: S 는 P.69 도해 + P.149(T 제도 방법)의 공통 절차로 실행 가능.
    //   T(어깨선 3.5 겹침 + 뒤 중심 0.5 올림)는 이번 증분 범위 밖이라 참고 슬롯으로 둔다.
    { id: "flat-collar", order: 4, label: "플랫 칼라", symbol: "S", page: 69, generator: "flat-collar-v1", availability: "available", note: null,
      variants: [
        { id: "bunka-flat-collar-S", symbol: "S", label: "S · 몸판에 직접 · 어깨 맞댐", page: 69, availability: "available", presetId: "bunka-flat-collar-S", note: null },
        { id: "bunka-flat-collar-T", symbol: "T", label: "T · 어깨선 3.5 겹침", page: 69, availability: "available", presetId: "bunka-flat-collar-T", note: null }
      ] },
    // 세일러 칼라 family: U·V·W 전부 P.70 도해 + P.150 제도 방법(하나의 생성기)으로 실행 가능.
    //   교재 본문: V = "칼라 폭을 조정하여 U와 같은 방식으로 제도한다" / W = "목둘레를 바꿔 U와 같이 제도한다".
    { id: "sailor-collar", order: 5, label: "세일러 칼라", symbol: "U", page: 70, generator: "sailor-collar-v1", availability: "available", note: null,
      variants: [
        { id: "bunka-sailor-collar-U", symbol: "U", label: "U · FNP 12 내림 · 어깨 폭 10", page: 70, availability: "available", presetId: "bunka-sailor-collar-U", note: null },
        { id: "bunka-sailor-collar-V", symbol: "V", label: "V · FNP 12 내림 · 어깨 폭 7", page: 70, availability: "available", presetId: "bunka-sailor-collar-V", note: null },
        { id: "bunka-sailor-collar-W", symbol: "W", label: "W · FNP 22 내림 · 어깨 폭 10", page: 70, availability: "available", presetId: "bunka-sailor-collar-W", note: null }
      ] },
    // 보 칼라 family: X·Y·Z 전부 P.71 도해(하나의 제도 — 목둘레를 수평선에 올린 직사각형 한 장)로 실행 가능.
    //   교재 본문: Y·Z = "Ⓧ와 같이 제도한다" — 칼라 폭과 리본 길이 두 수치만 다르다.
    { id: "bow-collar", order: 6, label: "보 칼라", symbol: "X", page: 71, generator: "bow-collar-v1", availability: "available", note: null,
      variants: [
        { id: "bunka-bow-collar-X", symbol: "X", label: "X · 칼라 폭 3 · 리본 45", page: 71, availability: "available", presetId: "bunka-bow-collar-X", note: null },
    ...
    // 테일러드 칼라 family: h·i 전부 P.78·79 도해 + P.152–153 제도 방법으로 실행 가능.
    //   교재가 "칼라 폭은 일정하게 두고 칼라 허리·누임 치수만 바꾼 2종류"라고 못박는다 — 하나의 생성기.
    { id: "tailored-collar", order: 9, label: "테일러드 칼라", symbol: "h", page: 78, generator: "tailored-collar-v1", availability: "available", note: null,
      variants: [
        { id: "bunka-tailored-collar-h", symbol: "h", label: "h · 칼라 허리 3 · 누임 2.5", page: 78, availability: "available", presetId: "bunka-tailored-collar-h", note: null },
        { id: "bunka-tailored-collar-i", symbol: "i", label: "i · 칼라 허리 1 · 누임 6", page: 79, availability: "available", presetId: "bunka-tailored-collar-i", note: null }
      ] },
    // 숄 칼라 family: j·k 전부 P.80·81 도해 + P.154–155 제도 방법으로 실행 가능.
    //   교재: "칼라 폭은 일정하게 두고 칼라 허리의 높이를 변경한 2종류" — 테일러드와 같은 구조.
    //   ★ 안단(겉 라펠 + 겉 위 칼라)은 앞 몸판을 포함하는 별도 조각이라 이번 범위 밖이다.
    { id: "shawl-collar", order: 10, label: "숄 칼라", symbol: "j", page: 80, generator: "shawl-collar-v1", availability: "available", note: null,
      variants: [
  ...
  var RECORDS = [
    ...
    {
      // 교재 S(P.69) — 본문 "앞뒤 각각의 몸판 목둘레선에 직접 칼라를 제도한다. 어깨를 맞대고 칼라를 베낀다."
      //   칼라 허리가 없는 플랫 칼라. 달림선 = 몸판 목둘레선 그대로(파생), 칼라 폭 5.5(뒤 중심·어깨),
      //   칼라 끝 = 앞 중심선에 평행한 안내선(4) 위에서 FNP 로부터 6.
      //   ★ 몸판 형상은 바꾸지 않는다 — 최종 bodice geometry 를 입력으로만 읽는다.
      id: "bunka-flat-collar-S", label: "교재 S", description: "플랫 칼라 S형(몸판 목둘레선에 직접 제도 · 어깨선에서 맞댐)",
      source: "『パターン製作の基礎』 플랫 칼라 S형(P.69) · 제도 방법(공통 절차) P.149",
      type: "flat-collar", baseMethod: "bunka-flat-collar-S-v1",
      neckline: { requiredType: "shirt", enforcement: "metadata-only" }, familyId: "flat-collar",
      flat: { collarWidthCm: 5.5, frontEndFromFnpCm: 6, frontEndOffsetCm: 4 }
    },
    {
      // 교재 T(P.69 하단) — 본문 "앞뒤 몸판의 어깨선을 겹쳐서 베끼고, 칼라를 제도한다.
      //   뒤 중심선에서 몸판 위로 0.5cm 낸 뒤 앞 중심선으로 칼라 달림선을 (그리면) 몸판의 목둘레
      //   치수보다 전체가 약 0.5cm 짧아진다. 부족분을 칼라를 달 때 늘려 박으면 (작은 칼라 허리를
      //   예쁘게 세울 수 있다)."
      //   겹침 3.5 = 두 어깨 끝점 사이 거리(P.149 ③). 칼라 폭 5.5(뒤 중심·어깨), 칼라 끝 6·4.
      //   ★ 몸판 형상은 바꾸지 않는다 — 최종 bodice geometry 를 입력으로만 읽는다.
      id: "bunka-flat-collar-T", label: "교재 T", description: "플랫 칼라 T형(어깨선 3.5 겹침 · 뒤 중심 0.5 올려 달림선 재작도)",
      source: "『パターン製作の基礎』 플랫 칼라 T형(P.69) · 제도 방법 P.149",
      type: "flat-collar-overlap", baseMethod: "bunka-flat-collar-T-v1",
      neckline: { requiredType: "shirt", enforcement: "metadata-only" }, familyId: "flat-collar",
      flatOverlap: { collarWidthCm: 5.5, cbRiseCm: 0.5, shoulderOverlapCm: 3.5, frontEndFromFnpCm: 6, frontEndOffsetCm: 4 }
    },
    {
      // 교재 U(P.70) — 본문 "목둘레는 FNP에서 12cm 내리고 칼라 폭은 어깨에서 10cm …
      //   플랫 칼라 T(P.69)와 같이 앞뒤 어깨선을 겹쳐서 베끼고, 칼라를 제도한다.
      //   칼라 외곽의 모양은 뒤에서 앞의 순서로 그린다."
      //   겹침 1.5 · 뒤 중심 올림 0.5 · 뒤 중심 폭 11 · 뒤 외곽 15.5(직각) · 어깨 폭 10 ·
      //   앞 외곽 휨 1.5 (P.150 제도 방법). V 목둘레(12·0.8)는 칼라 내부 파생.
      //   ★ 몸판 형상은 바꾸지 않는다 — 최종 bodice geometry 를 입력으로만 읽는다.
      id: "bunka-sailor-collar-U", label: "교재 U", description: "세일러 칼라 U형(V 목둘레 12 · 어깨 칼라 폭 10 · 어깨선 1.5 겹침)",
      source: "『パターン製作の基礎』 세일러 칼라 U형(P.70) · 제도 방법 P.150",
      type: "sailor-collar", baseMethod: "bunka-sailor-collar-U-v1",
      neckline: { requiredType: "shirt", enforcement: "metadata-only" }, familyId: "sailor-collar",
      sailor: { vDropCm: 12, vHollowCm: 0.8, shoulderOverlapCm: 1.5, cbRiseCm: 0.5,
        cbWidthCm: 11, backOuterCm: 15.5, shoulderWidthCm: 10, frontOuterBowCm: 1.5 }
    },
    {
      // 교재 V(P.70) — 본문 "칼라 폭이 좁은 세일러 칼라. 칼라 폭을 조정하여 Ⓤ와 같은 방식으로 제도한다.
      //   어깨의 칼라 폭을 좁게 하는 경우, 뒤 중심의 길이도 짧게 해야 균형이 맞는다. 반대로 칼라 폭을
      //   넓게 할 때는 뒤 중심도 마찬가지로 길게 한다." · 도해 설명 "칼라 허리와 목둘레는 Ⓤ와 같다."
      //   → 제도 절차(P.150)·각 치수의 시작점·끝점·방향은 U 와 **완전히 같고 수치만 다르다**:
      //   목둘레 12·휨 0.8·겹침 1.5·뒤 중심 올림 0.5 = U 동일 / 폭 계열만 축소
      //   (뒤 중심 11→9 · 뒤 외곽 15.5→13.5 · 어깨 폭 10→7 · 앞 외곽 휨 1.5→1).
      //   ★ 몸판 형상은 바꾸지 않는다 — 최종 bodice geometry 를 입력으로만 읽는다.
      id: "bunka-sailor-collar-V", label: "교재 V", description: "세일러 칼라 V형(V 목둘레 12 · 어깨 칼라 폭 7 · 뒤 중심 9 로 균형)",
      source: "『パターン製作の基礎』 세일러 칼라 V형(P.70) · 제도 방법 P.150",
      type: "sailor-collar", baseMethod: "bunka-sailor-collar-V-v1",
      neckline: { requiredType: "shirt", enforcement: "metadata-only" }, familyId: "sailor-collar",
      sailor: { vDropCm: 12, vHollowCm: 0.8, shoulderOverlapCm: 1.5, cbRiseCm: 0.5,
        cbWidthCm: 9, backOuterCm: 13.5, shoulderWidthCm: 7, frontOuterBowCm: 1 }
    },
    {
      // 교재 W(P.70) — 본문 "앞 목둘레 V넥을 깊게 한 디자인. 목둘레를 바꿔 Ⓤ와 같이 제도한다."
      //   도해 설명 "V넥이 깊고 앞 칼라 외곽은 직선적이다."
      //   → 제도 절차(P.150)·기준점은 U 와 같고, **목둘레와 앞 외곽 휨만** 다르다:
      //   FNP 내림 12→22 · V선 휨 0.8→0.3 · 앞 외곽 휨 1.5→0.7 / 폭 계열(11·15.5·10)·겹침 1.5·올림 0.5 = U 동일.
      //   ★ 몸판 형상은 바꾸지 않는다 — V 목둘레는 칼라 안에서만 파생한다.
      id: "bunka-sailor-collar-W", label: "교재 W", description: "세일러 칼라 W형(V 목둘레 22 로 깊게 · 어깨 칼라 폭 10 · 앞 외곽 직선적)",
      source: "『パターン製作の基礎』 세일러 칼라 W형(P.70) · 제도 방법 P.150",
      type: "sailor-collar", baseMethod: "bunka-sailor-collar-W-v1",
      neckline: { requiredType: "shirt", enforcement: "metadata-only" }, familyId: "sailor-collar",
      sailor: { vDropCm: 22, vHollowCm: 0.3, shoulderOverlapCm: 1.5, cbRiseCm: 0.5,
        cbWidthCm: 11, backOuterCm: 15.5, shoulderWidthCm: 10, frontOuterBowCm: 0.7 }
    },
    {
      // 교재 X(P.71) 보 칼라 — 본문 "앞 몸판의 칼라 달림 끝은 리본 매듭이 예쁘게 자리 잡도록 앞 중심에서
      //   떨어진 위치에 정한다. 목둘레 치수를 수평선상에 두고 제도한다. 칼라 달림 끝부터 리본의 길이는 45cm."
      //   도해: 직사각형 한 장(왼쪽 변 = 뒤 중심) · 아래 변 = (×+⊠)+45 · 칼라 폭 3.
      //   ⊠ = SNP→칼라 달림 끝, 3 = 칼라 달림 끝→앞 중심(둘 다 목둘레선을 따라 잰 값).
      id: "bunka-bow-collar-X", label: "교재 X", description: "보 칼라 X형(칼라 폭 3 · 리본 45 · 칼라 달림 끝 앞 중심에서 3)",
      source: "『パターン製作の基礎』 보 칼라 X형(P.71)",
      type: "bow-collar", baseMethod: "bunka-bow-collar-X-v1",
      neckline: { requiredType: "shirt", enforcement: "metadata-only" }, familyId: "bow-collar",
      bow: { collarWidthCm: 3, ribbonLengthCm: 45, attachEndFromCfCm: 3 }
    },
    {
      // 교재 Y(P.71) — 본문 "칼라 폭 7cm. Ⓧ와 같이 제도한다. 칼라 달림 끝부터 리본의 길이는 60cm."
      id: "bunka-bow-collar-Y", label: "교재 Y", description: "보 칼라 Y형(칼라 폭 7 · 리본 60 · 제도는 X와 같다)",
      source: "『パターン製作の基礎』 보 칼라 Y형(P.71)",
      type: "bow-collar", baseMethod: "bunka-bow-collar-Y-v1",
      neckline: { requiredType: "shirt", enforcement: "metadata-only" }, familyId: "bow-collar",
      bow: { collarWidthCm: 7, ribbonLengthCm: 60, attachEndFromCfCm: 3 }
    },
    {
      // 교재 Z(P.71) — 본문 "칼라 폭 15cm. Ⓧ와 같이 제도한다. 칼라 달림 끝부터 리본의 길이는 75cm."
      id: "bunka-bow-collar-Z", label: "교재 Z", description: "보 칼라 Z형(칼라 폭 15 · 리본 75 · 제도는 X와 같다)",
      source: "『パターン製作の基礎』 보 칼라 Z형(P.71)",
      type: "bow-collar", baseMethod: "bunka-bow-collar-Z-v1",
      neckline: { requiredType: "shirt", enforcement: "metadata-only" }, familyId: "bow-collar",
    ...
    {
      // 교재 h(P.78) 테일러드 칼라 — 본문 "테일러드 칼라의 기본형. 몸판의 목둘레, 라펠, 위 칼라의 순으로
      //   제도한다. 꺾임 끝은 BL과 WL 사이를 2등분한 위치로. 칼라 폭은 뒤에서 4cm, 라펠에서 8cm,
      //   각진 부분은 3cm, 누임 치수(★)는 2.5cm로 설정한다."
      //   목둘레 이동량 1 · 앞 목둘레선 2 · 앞 중심 올림 1 · 라펠 외형 0.5 는 P.152 도해 표기.
      //   1-❸ 표기 2.3 = 칼라 허리 3 − 어깨선 연장 0.7 (엔진이 파생 — 레코드 수치 아님).
      id: "bunka-tailored-collar-h", label: "교재 h", description: "테일러드 칼라 h형(칼라 허리 3 · 누임 2.5 · 라펠 폭 8)",
      source: "『パターン製作の基礎』 테일러드 칼라 h형(P.78) · 제도 방법 P.152–153",
      type: "tailored-collar", baseMethod: "bunka-tailored-collar-h-v1",
      neckline: { requiredType: "shirt", enforcement: "metadata-only" }, familyId: "tailored-collar",
      tailored: { collarStandCm: 3, collarWidthCm: 4, lapelWidthCm: 8, layDownCm: 2.5, neckShiftCm: 1,
        tipRadiusCm: 3, lapelBowCm: 0.5, frontNeckCm: 2, frontRiseCm: 1 }
    },
    {
      // 교재 i(P.79) — 본문 "Ⓗ와 같이 제도한다. **칼라 허리를 낮게 설정하면 칼라 외곽의 치수가 많이
      //   필요하다. 그 치수를 확보하기 위해 누임 치수를 6cm로 늘려서** 칼라를 그린다."
      //   즉 허리 1 과 누임 6 은 짝이다. 목둘레 이동량만 0.6 으로 함께 바뀐다.
      id: "bunka-tailored-collar-i", label: "교재 i", description: "테일러드 칼라 i형(칼라 허리 1 · 누임 6 으로 외곽 치수 확보)",
      source: "『パターン製作の基礎』 테일러드 칼라 i형(P.79) · 제도 방법 P.152–153",
      type: "tailored-collar", baseMethod: "bunka-tailored-collar-i-v1",
      neckline: { requiredType: "shirt", enforcement: "metadata-only" }, familyId: "tailored-collar",
      tailored: { collarStandCm: 1, collarWidthCm: 4, lapelWidthCm: 8, layDownCm: 6, neckShiftCm: 0.6,
        tipRadiusCm: 3, lapelBowCm: 0.5, frontNeckCm: 2, frontRiseCm: 1 }
    },
    {
      // 교재 j(P.80) 숄 칼라 — 본문 "숄 칼라의 기본형. 몸판의 목둘레, 라펠, 위 칼라의 순으로 제도한다.
      //   꺾임 끝은 BL과 WL 사이를 2등분한 위치로, 칼라 폭은 뒤에서 4cm, 라펠에서 6cm,
      //   누임 치수(★)는 2.5cm로 설정."  테일러드 h 와 라펠 폭(8→6)·깃아귀 유무만 다르다.
      id: "bunka-shawl-collar-j", label: "교재 j", description: "숄 칼라 j형(칼라 허리 3 · 누임 2.5 · 라펠 폭 6 · 깃아귀 없음)",
      source: "『パターン製作の基礎』 숄 칼라 j형(P.80) · 제도 방법 P.154–155",
      type: "shawl-collar", baseMethod: "bunka-shawl-collar-j-v1",
      neckline: { requiredType: "shirt", enforcement: "metadata-only" }, familyId: "shawl-collar",
      shawl: { collarStandCm: 3, collarWidthCm: 4, lapelWidthCm: 6, layDownCm: 2.5, neckShiftCm: 1,
        frontNeckCm: 2, frontRiseCm: 1 }
    },
    {
      // 교재 k(P.81) — 본문 "Ⓙ와 같이 제도한다. 칼라 허리를 낮게 설정하면 칼라 외곽의 치수가 많이
      //   필요하다. 이 치수를 확보하기 위해 누임 치수를 6cm로 늘려서 칼라를 그린다."(테일러드 i 와 같은 인과)
      id: "bunka-shawl-collar-k", label: "교재 k", description: "숄 칼라 k형(칼라 허리 1 · 누임 6 으로 외곽 치수 확보)",
      source: "『パターン製作の基礎』 숄 칼라 k형(P.81) · 제도 방법 P.154–155",
      type: "shawl-collar", baseMethod: "bunka-shawl-collar-k-v1",
      neckline: { requiredType: "shirt", enforcement: "metadata-only" }, familyId: "shawl-collar",
      shawl: { collarStandCm: 1, collarWidthCm: 4, lapelWidthCm: 6, layDownCm: 6, neckShiftCm: 0.6,
        frontNeckCm: 2, frontRiseCm: 1 }
    },
    {
  ...
  var RECORD_TYPES = { "shirt-two-piece": 1, "shirt-one-piece": 1, "shirt-open-collar": 1, "shirt-wing-collar": 1, "shirt-band-one-piece": 1, "flat-collar": 1, "flat-collar-overlap": 1, "sailor-collar": 1, "bow-collar": 1, "frill-collar": 1, "hood": 1, "tailored-collar": 1, "shawl-collar": 1, "stand-collar": 1 };
  function validateRecord(r) {
    ...
    if (r.type === "shirt-one-piece") {
      ...
      ["stand", "body", "openCollar", "tip", "upper", "flat", "flatOverlap", "sailor", "bow", "hood", "tailored", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
    ...
    if (r.type === "sailor-collar") {
      ...
      ["stand", "body", "onePiece", "openCollar", "standalone", "construction", "tip", "upper", "flat", "flatOverlap", "bow", "hood", "tailored", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
      validateSection(r.sailor, SAILOR_COLLAR_FIELDS, "sailor", r.id);
      return true;
    }
    if (r.type === "bow-collar") {
      // 보 칼라: bow 섹션 하나뿐. 밴드·위 칼라·플랫·세일러 섹션을 빌려오지 않는다.
      ["stand", "body", "onePiece", "openCollar", "standalone", "construction", "tip", "upper", "flat", "flatOverlap", "sailor", "hood", "tailored", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
      validateSection(r.bow, BOW_COLLAR_FIELDS, "bow", r.id);
      return true;
    }
    if (r.type === "frill-collar") {
      ["stand", "body", "onePiece", "openCollar", "standalone", "construction", "tip", "upper", "flat", "flatOverlap", "sailor", "bow", "hood", "tailored", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
    ...
    if (r.type === "hood") {
      ...
      ["stand", "body", "onePiece", "openCollar", "standalone", "construction", "tip", "upper", "flat", "flatOverlap", "sailor", "bow", "frill", "tailored", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
    ...
    if (r.type === "tailored-collar") {
      // 테일러드 칼라: tailored 섹션 하나뿐.
      ["stand", "body", "onePiece", "openCollar", "standalone", "construction", "tip", "upper", "flat", "flatOverlap", "sailor", "bow", "frill", "hood", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
      validateSection(r.tailored, TAILORED_FIELDS, "tailored", r.id);
      // 엔진 계약: 1-❸(칼라 허리 − 어깨선 연장 0.7) > 0
      if (!(r.tailored.collarStandCm > 0.7)) fail("out-of-range", r.id + ".tailored.collarStandCm");
      return true;
    }
    if (r.type === "shawl-collar") {
      // 숄 칼라: shawl 섹션 하나뿐. 테일러드의 tailored 섹션(깃아귀 수치 포함)을 빌려오지 않는다.
      ["stand", "body", "onePiece", "openCollar", "standalone", "construction", "tip", "upper", "flat", "flatOverlap", "sailor", "bow", "frill", "hood", "tailored"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
      validateSection(r.shawl, SHAWL_FIELDS, "shawl", r.id);
      if (!(r.shawl.collarStandCm > 0.7)) fail("out-of-range", r.id + ".shawl.collarStandCm");   // 1-❸ > 0
      return true;
    }
    if (r.type === "flat-collar-overlap") {
      // 플랫 칼라 T: flatOverlap 섹션 하나뿐(S 의 flat 과도 섞지 않는다).
      ["stand", "body", "onePiece", "openCollar", "standalone", "construction", "tip", "upper", "flat", "sailor", "bow", "hood", "tailored", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
    ...
    if (r.type === "flat-collar") {
      ...
      ["stand", "body", "onePiece", "openCollar", "standalone", "construction", "tip", "upper", "flatOverlap", "sailor", "bow", "hood", "tailored", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
    ...
    if (r.type === "shirt-band-one-piece") {
      ...
      ["body", "onePiece", "openCollar", "standalone", "construction", "tip", "flat", "flatOverlap", "sailor", "bow", "hood", "tailored", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
    ...
    if (r.type === "shirt-wing-collar") {
      ...
      ["body", "onePiece", "openCollar", "standalone", "construction", "upper", "flat", "flatOverlap", "sailor", "bow", "hood", "tailored", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
      validateSection(r.stand, STAND_FIELDS, "stand", r.id);
    ...
    if (r.type === "shirt-open-collar") {
      ...
      ["stand", "body", "onePiece", "standalone", "tip", "upper", "flat", "flatOverlap", "sailor", "bow", "hood", "tailored", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
    ...
    if (r.type === "stand-collar") {
      ["stand", "body", "onePiece", "openCollar", "tip", "upper", "flat", "flatOverlap", "sailor", "bow", "hood", "tailored", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
    ...
    ["onePiece", "openCollar", "standalone", "tip", "upper", "flat", "flatOverlap", "sailor", "bow", "hood", "tailored", "shawl"].forEach(function (k) { if (k in r) fail("mixed-record-sections", r.id + "." + k); });
  ...
  function defaults(id) {
    ...
    if (r.type === "flat-collar") return { ok: true, id: r.id, type: r.type, flat: clone(r.flat) };
    ...
    if (r.type === "tailored-collar") return { ok: true, id: r.id, type: r.type, tailored: clone(r.tailored) };
    if (r.type === "shawl-collar") return { ok: true, id: r.id, type: r.type, shawl: clone(r.shawl) };
  ...
  // 섹션 필드 의미·단위(표시용).
  function fields(section) { return clone(section === "stand" ? STAND_FIELDS : section === "body" ? BODY_FIELDS : section === "onePiece" ? ONE_PIECE_FIELDS : section === "openCollar" ? OPEN_COLLAR_FIELDS : section === "tip" ? WING_TIP_FIELDS : section === "upper" ? UPPER_ONE_PIECE_FIELDS : section === "flat" ? FLAT_COLLAR_FIELDS : section === "flatOverlap" ? FLAT_COLLAR_T_FIELDS : section === "sailor" ? SAILOR_COLLAR_FIELDS : section === "bow" ? BOW_COLLAR_FIELDS : section === "frill" ? FRILL_COLLAR_FIELDS : section === "hood" ? HOOD_FIELDS : section === "tailored" ? TAILORED_FIELDS : section === "shawl" ? SHAWL_FIELDS : section === "standalone" ? STANDALONE_FIELDS : []); }
  ...
  function matches(id, stand, body) {
    ...
    if (r.type === "flat-collar") return eq(stand, r.flat, FLAT_COLLAR_FIELDS);
    ...
    if (r.type === "tailored-collar") return eq(stand, r.tailored, TAILORED_FIELDS);
    if (r.type === "shawl-collar") return eq(stand, r.shawl, SHAWL_FIELDS);
  ...
  function composeDraft(id, bodice, DC) {
    ...
    if (r.type === "sailor-collar") {
      // 앞뒤 어깨선을 겹쳐 놓고 V 목둘레(내부 파생) 위에 세일러 칼라를 그린다.
      var sailorRe = DC.computeSailorCollarU(bodice, d.sailor);
      if (!sailorRe.ok) return { ok: false, stage: "collar", reason: sailorRe.reason };
      return { ok: true, draft: {
        sourceBodiceHash: bodice.hash, type: r.type, baseMethod: r.baseMethod, presetId: r.id,
        parameters: { sailor: d.sailor },
        sailor: { geometry: sailorRe.geometry, measure: sailorRe.measure, anchors: sailorRe.anchors }
      } };
    }
    if (r.type === "bow-collar") {
      // 목둘레 치수(×+⊠)를 수평선에 올려 직사각형 한 장으로 제도한다(몸판은 바꾸지 않는다).
      var bowRe = DC.computeBowCollar(bodice, d.bow);
      if (!bowRe.ok) return { ok: false, stage: "collar", reason: bowRe.reason };
      return { ok: true, draft: {
        sourceBodiceHash: bodice.hash, type: r.type, baseMethod: r.baseMethod, presetId: r.id,
    ...
    if (r.type === "tailored-collar") {
      // 앞 몸판 위에 라펠 → 칼라 순으로 제도한다. 라펠·몸판 목둘레선은 bodyLink 로 파생 기록만 한다.
      var tlRe = DC.computeTailoredCollar(bodice, d.tailored);
      if (!tlRe.ok) return { ok: false, stage: "collar", reason: tlRe.reason };
      return { ok: true, draft: {
        sourceBodiceHash: bodice.hash, type: r.type, baseMethod: r.baseMethod, presetId: r.id,
        parameters: { tailored: d.tailored },
        tailored: { geometry: tlRe.geometry, measure: tlRe.measure, anchors: tlRe.anchors, bodyLink: tlRe.bodyLink }
      } };
    }
    if (r.type === "shawl-collar") {
      // 테일러드와 같은 1~4 단계 위에, 깃아귀 없이 외곽을 한 줄기 곡선으로 잇는다.
      var swRe = DC.computeShawlCollar(bodice, d.shawl);
      if (!swRe.ok) return { ok: false, stage: "collar", reason: swRe.reason };
      return { ok: true, draft: {
        sourceBodiceHash: bodice.hash, type: r.type, baseMethod: r.baseMethod, presetId: r.id,
        parameters: { shawl: d.shawl },
        shawl: { geometry: swRe.geometry, measure: swRe.measure, anchors: swRe.anchors, bodyLink: swRe.bodyLink }
      } };
    }
    if (r.type === "flat-collar-overlap") {
      // 앞뒤 어깨선을 3.5 겹쳐 놓고 한 장으로 제도한다(달림선은 0.5 올려 재작도).
      var flatTRe = DC.computeFlatCollarT(bodice, d.flatOverlap);
      if (!flatTRe.ok) return { ok: false, stage: "collar", reason: flatTRe.reason };
      return { ok: true, draft: {
        sourceBodiceHash: bodice.hash, type: r.type, baseMethod: r.baseMethod, presetId: r.id,
        parameters: { flatOverlap: d.flatOverlap },
        flat: { geometry: flatTRe.geometry, measure: flatTRe.measure, anchors: flatTRe.anchors }
      } };
    }
    if (r.type === "flat-collar") {
      // 몸판 목둘레선에 직접 그린 앞·뒤 칼라를 어깨선에서 맞대어 한 장으로 만든다.
      var flatRe = DC.computeFlatCollarS(bodice, d.flat);
      if (!flatRe.ok) return { ok: false, stage: "collar", reason: flatRe.reason };
      return { ok: true, draft: {
        sourceBodiceHash: bodice.hash, type: r.type, baseMethod: r.baseMethod, presetId: r.id,
        parameters: { flat: d.flat },
        flat: { geometry: flatRe.geometry, measure: flatRe.measure, anchors: flatRe.anchors }
      } };
    }
    if (r.type === "shirt-band-one-piece") {
  ...
  // presetId → 그 프리셋을 가리키는 catalog variant(동결) 또는 null.
  //   표시 문구가 **적용된 초안의 교재 표식**(선택 select 가 아니라 collarDraft.presetId)을 써야 할 때 쓴다.
  //   같은 생성기를 공유하는 계열(예: 세일러 U·V·W)에서 어느 도해인지 문구에 남기는 용도 — 형상·hash 와 무관.
  function variantByPreset(presetId) {
    if (!presetId) return null;
    for (var i = 0; i < CAT.list.length; i++) {
      var vs = CAT.list[i].variants;
      for (var j = 0; j < vs.length; j++) if (vs[j].presetId === presetId) return vs[j];
    }
    return null;
  }
  ...
  window.collarPresets = Object.freeze({
    ...
    familyOptions: familyOptions, variantOptions: variantOptions, variantByPreset: variantByPreset, resolve: resolve, referenceRows: referenceRows, displayTitle: displayTitle, referenceFields: function () { return clone(REFERENCE_FIELDS); },
