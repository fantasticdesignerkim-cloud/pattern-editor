...
// 숄 칼라(교재 j·k, collarDraft.shawl): 위 칼라 한 조각(깃아귀 없음) + 몸판 파생(라펠·꺾임선).
function _appendCollarShawl(root, collarDraft, off, scale){
  const sw=collarDraft && collarDraft.shawl;
  if(!sw || !sw.geometry || !Array.isArray(sw.geometry.outline)) return;
  const d=_partPathD(sw.geometry.outline); if(!d) return;
  const g=E("g",{ transform:"translate("+(off.dx*scale)+","+(off.dy*scale)+")", "data-design-collar":"shawl" });
  g.appendChild(E("path",{ d, class:"design-collar-onepiece", fill:"none" }));
  const sc=_partPathD(sw.geometry.construction||[]);
  if(sc) g.appendChild(E("path",{ d:sc, class:"design-collar-fold", fill:"none" }));
  root.appendChild(g);
}
...
function render(){
  ...
  if (designActive) {
    ...
    if (dp.working.collarDraft && (dp.working.collarDraft.standGeometry || dp.working.collarDraft.body || dp.working.collarDraft.onePiece || dp.working.collarDraft.openCollar || dp.working.collarDraft.tip || dp.working.collarDraft.joined || dp.working.collarDraft.flat || dp.working.collarDraft.sailor || dp.working.collarDraft.bow || dp.working.collarDraft.frill || dp.working.collarDraft.hood || dp.working.collarDraft.tailored || dp.working.collarDraft.shawl || dp.working.collarDraft.standalone)) {
      ...
      _appendCollarShawl(cRoot, dp.working.collarDraft, cOff, scale);
