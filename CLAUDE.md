...
제거, 외곽선 재연결, 열린 웨지 유지). **아직 미구현 — 다음 작업.** 현재는 델타
안전망(2026-07-07)이 잘못된 결과의 커밋만 막고 있음(정리는 안 함).
...
6. **델타 안전망 (적용 직전)**: 이 회전이 **각도 0 대비 자기교차를 새로 늘리면**
   적용 거부. `findRotationCollisions`가 못 잡는, bake가 새로 만드는 잔여벽(legOld)
   관통까지 걸러냄. 각도0 재조립(baked0)을 기준선으로 삼아 비교하므로 원본부터
   있던 노이즈는 상쇄 → 실사용 오차단 0(순차 다트이동은 새 겹침 0건).
...
**69→7건**. 남은 7건은 **각도0에서도 존재하는 bake 조립 아티팩트**(비현실적 조각
선택 시 잔여벽 관통) — 다세대 조립 로직 클린 재설계 영역, 오늘은 원칙대로 멈춤.
**실제 UI 순차 다트이동(진동선 오른쪽부터, 어깨쪽 조각, 풀클로징)은 완전히 깨끗**:
breaks 0 / loopGap 0 / 자기교차 0 / 중복다리 0. 캐시 버전 `?v=2026070710`.

**열린 이슈**: 위 7건(비현실적 조각 선택 시 legOld가 몸판 관통) — `bakeFromSplitPieces`
의 다세대 잔여벽 조립을 클린 스펙으로 재설계할 때 처리. 지금은 델타 안전망이
커밋을 막아주므로 잘못된 결과가 저장되진 않음.
...
- **(다음 작업, 확정) `normalizeBakedSegments` 구현** — 지배 모델 "젤리/물 = 현재
  조각 하나"(위 아키텍처 최상단). apply 직후: 폭 0 닫힌 다트쌍·과거 세대 왕복
  스파이크·중복 old-dart 제거하고 외곽선 재연결, 열린 다트 웨지만 유지. 이러면
  아래 "잔여 7건"(비현실적 조각 선택 시 legOld 몸판 관통)이 근본 해소되고, 델타
  안전망은 "막기→정리"로 격상. split/bake가 매번 깨끗한 현재 형상에서 시작.
- **뒤판(back) 회전 배리어 미적용**: 이번 `rotationLegBarrier`/`cutPoint` 전달은
  앞/뒤 공용 selectPiece 핸들러라 뒤판도 타지만, 뒤판 중심 스트레스 검증은 아직
  안 함. 뒤판 다중다트 확인 필요.
