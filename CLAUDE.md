...
- **(다음 작업) 형상 엔진 재설계 — 순서 ③의 C7** — 위 "형상 엔진 재설계 스펙" 참고.
  **C0~C6 완료.** ①(`evaluateEndpoint`)·②(`findPhysicalSweepLimit`)가 배선됐고, 부호는
  `findMaxApplicableMagnitude` 위→아래 탐색(`selectRotationSign`), 드래그 요청각은 ④
  (`resolveRequestedAngle`)가 비단조 스냅으로 확정하며, preview·apply는 ④의
  `evaluation.shape`를 `getReusableDartEvaluation()`로 공유한다(위 "C6 완료" 참고).
  **다음은 C7 `Reduce apply-time gates to single source of truth`. 착수 승인 대기.**
  **C7 경계 (반드시 지킬 것)**:
  - **제거**: C1 임시 mismatch throw(`[C1] endpoint evaluation mismatch`) + apply-time
    중복 게이트(delta selfX = ①의 self-intersection, budget = ①의 budget-exceeded).
    단일 경로 `if (!ev?.valid) rejectMove(ev.reasons); commitMove(ev)`.
  - **유지(제거 금지)**: (a) resolved 각도의 `evaluateEndpoint` **valid 단일 차단** —
    격자가 스텝보다 좁은 금지구간을 놓칠 수 있음(C5 재확인). (b) **MIN 게이트**(퇴화 다트).
    (c) **② piece-collision**(`findRotationCollisions`) — ①의 계약상 evaluation.valid에
    없으므로 별도 유지.
  - **`index.html` 캐시 버전**: C7 완료 시 한 번만 갱신(개발 중엔 하드 리프레시).
  - **성능**: bake는 병목 아님 — **②의 rotColl 60스텝 스캔이 지배**. `≤1.2×` 넘으면
