...
```
evaluateEndpoint(θ)          // 특정 각도의 bake 결과·예산·자기교차 판정 (끝점 원자)
findPhysicalSweepLimit(0→θ)  // 실제 고정×회전 조각 충돌 경로 검사 (스윕)
findApplicableIntervals(0→limit)  // endpoint가 적용 가능한 "각도 구간 목록"
resolveRequestedAngle(req)   // 물리 한계 안에서 가장 가까운 적용 가능 각도로 스냅
```
**비단조라 "최대 안전각 하나"로는 부족하다** — 5°는 불가능하고 18°는 다시 가능할 수
있으므로 안전구간을 **구간 목록**으로 표현한다:
```
applicableIntervals: [ { from: 0, to: 0.75 }, { from: 17.8, to: 18.06 } ]
```
...
고정(**✅ 앞/뒤 골든 완료**) → ② 한 파일 안에서 순수/UI 함수 분리 → ③ `evaluateMove`
(4계층)로 모든 게이트 통합 → ④ geometry/topology 파일 추출 → ⑤ UI 컨트롤러 얇게
정리 → ⑥ 안정화 후 bake가 처음부터 깨끗한 형상을 만들도록 개선 → ⑦ normalize를
청소기에서 최종 검증기로 축소. **성능 최적화는 맨 마지막**(각도별 평가 캐시·세그먼트
bounding-box 사전검사·동일 bake 중복 제거). 3다트 골든은 현재 post-normalize를 잠그되
**영구 정답이 아니라 리팩터 기간의 호환 기준** — ⑥에서 아티팩트를 근본 개선할 때
`--update`와 변경 사유를 함께 커밋한다.
...
- **(다음 작업) 형상 엔진 재설계** — 위 "형상 엔진 재설계 스펙" 섹션 참고. 순서 ①
  회귀 고정은 완료, **다음은 ② 한 파일 안에서 순수/UI 함수 분리**. 파일을 아직
  안 쪼개고 `dartMove.js` 내부에서 geometry/topology/engine/controller 구역만 먼저
  나눈다. 착수 승인 대기.
- 파일 분리·class화는 위 재설계 순서 ④에서 다룬다(회귀 골든이 잠겼으므로 이제 진행 가능).
