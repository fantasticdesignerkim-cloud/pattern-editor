...
| **✅ C5** | `Resolve requested drag angle…` (`5cf9af6`) + `Move dart-move harness off legacy…` (`f542abc`) + `Remove legacy dart angle chain…` (`0e58b82`) | ④ + clamp 교체 + legacy 순수 삭제 | 무변경(sign-selection.json 삭제) |
| C6 | `Share evaluated shape between preview and apply` | `commitMove` | 무변경 |
| C7 | `Reduce apply-time gates to single source of truth` | 중복 제거(차단은 유지) | 무변경 |
...
- **C5d** (`0e58b82`): 순수 삭제. `findMaxSafeAngle`/`budgetMaxAngle`/`applyTimeSafeAngle`/
  ...
  `signSelectionMigration.js`/`signSelectionFixture.js`(+`sign-selection.json`) + 죽은
  `requestedAngleRad`. `rotationSignSelection.js`는 프로덕션 ③를 oracle로 쓰던 3곳을
  독립 oracle `scanIntervals`로 교체("프로덕션으로 프로덕션 검증" 냄새 제거).
...
- **(다음 작업) 형상 엔진 재설계 — 순서 ③의 C5** — 위 "형상 엔진 재설계 스펙" 참고.
  **C0·C1·C2·C3·C4·C5 완료.** ①(`evaluateEndpoint`)·②(`findPhysicalSweepLimit`)가 프로덕션
  경로에 배선됐고, 부호 선택은 `findMaxApplicableMagnitude` 위→아래 탐색으로 통합됐으며
  (`selectRotationSign`), 드래그 요청각은 ④(`resolveRequestedAngle`)가 비단조 스냅으로
  확정한다. legacy 부호 체인·③ 구간 열거는 C5d에서 전부 삭제됨(위 "C5 완료" 참고).
  **다음은 C6 `Share evaluated shape between preview and apply`** — `commitMove` 신설,
  preview와 apply가 같은 `evaluateEndpoint` shape를 공유(중복 bake 제거). **착수 승인 대기.**
  **C6/C7에서 이어질 것**:
  1. **④의 `evaluation`을 commit에 재사용** — ④가 이미 resolved 각도의 shape를 평가해
     `evaluation.shape`를 갖고 있다. C6에서 apply가 이걸 다시 bake하지 않고 공유하면
     C1의 임시 이중 검증(`[C1] endpoint evaluation mismatch` throw)이 불필요해진다.
  2. **C7: C1 임시 throw 제거 + apply-time 게이트 단일화** — `if(!ev?.valid) rejectMove;
     commitMove(ev)` 단일 경로. **단, resolved 각도의 `evaluateEndpoint` 단일 차단은
     제거 금지**(격자가 스텝보다 좁은 금지구간을 놓칠 수 있으므로 — C5에서 재확인).
  3. **`index.html` 캐시 버전**: C7 완료 시 한 번만 갱신(개발 중엔 하드 리프레시).
  4. **성능**: bake는 병목 아님 — **②의 rotColl 60스텝 스캔이 지배**. `≤1.2×` 넘으면
     스텝 줄이지 말고 원인 실측 보고. perf 골든은 shape 골든과 규칙이 다르다.
- **✅ legacy 순수 삭제 완료** — `chooseSignedBaseAngle`·`budgetMaxAngle`·
  `applyTimeSafeAngle`·`findMaxSafeAngle`·`findApplicableIntervals`는 C5d에서 삭제.
  위 "Dead code 감사"의 `calc*CloseAngleByRotateHit` 3종은 이미 프로덕션에 없음(실측 확인).
- 파일 분리·class화는 위 재설계 순서 ④에서 다룬다(회귀 골든 + 구역 경계가 섰으므로 진행 가능).
