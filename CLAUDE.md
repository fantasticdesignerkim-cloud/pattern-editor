...
| C4 | `Choose rotation sign from applicable intervals` | `chooseSignedBaseAngle` 해체 | 무변경 |
| C5 | `Resolve requested drag angle against applicable intervals` | ④ + clamp 교체 | 무변경 |
...
**★ C4/C5 배선 때 쓸 성능 재원 (C3에서 발견, 이번엔 손대지 않음)**:
`evaluateEndpoint`가 **매 호출마다 `findSelfIntersections(ctx.prevBakedSegments)`로
기준선을 다시 계산한다**(`baselineSelfXCount`). 그런데 이 기준선은 **스캔 내내 불변**이다 —
115회 스캔이면 114회가 낭비다. 배선 시 ctx 수준에서 한 번만 구해 넘기면 캐시 없이
구조로 제거된다. 이건 문서가 예견한 "같은 샘플 그리드를 공유하면 사라지는 중복"
(`budgetMaxAngle`·`applyTimeSafeAngle`이 각자 그리드에서 따로 bake하는 것)과 **별개의
추가 재원**이다. C3 범위(추가만)라 의도적으로 안 고쳤다.
...
- **(다음 작업) 형상 엔진 재설계 — 순서 ③의 C4** — 위 "형상 엔진 재설계 스펙" 섹션 참고.
  **C0·C1·C2·C3 완료** — 4계층 중 ①(`evaluateEndpoint`)·②(`findPhysicalSweepLimit`)·
  ③(`findApplicableIntervals`)가 전부 존재하고 각각 픽스처로 검증돼 있다. **단 ③까지는
  아직 프로덕션에 배선되지 않았다** — `prepareDartMoveCandidate`는 여전히
  `findMaxSafeAngle → budgetMaxAngle → applyTimeSafeAngle`를 탄다.
  **다음은 C4 `Choose rotation sign from applicable intervals`**: `chooseSignedBaseAngle`을
  해체해 두 부호 각각의 ③ 구간 목록을 비교하는 방식으로 교체한다. `prepareDartMoveCandidate`의
  `limits`가 gen-0 경로에서 null인 건 세 캡이 `chooseSignedBaseAngle` 내부에 갇혀 있어서인데,
  C4가 그걸 밖으로 꺼내면 채워진다.
  **C4/C5 배선 시 반드시 지킬 것**: (1) `≤1.2×` 성능 판정을 여기서 내린다 — 넘으면 스텝을
  줄이지 말고 **평가 중복부터 제거**(위 C3 섹션의 "성능 재원" 두 가지: 각자 그리드 bake 중복 +
  `evaluateEndpoint`의 기준선 재계산). (2) 구간을 찾았어도 **최종 차단은 정확한 요청각의
  `evaluateEndpoint`**가 한다(격자는 스텝보다 좁은 금지구간을 보장 탐지 못 함 — C7에서도 제거 금지).
  (3) perf 골든은 shape 골든과 규칙이 다르다(위 "골든 JSON 변경" 항목).
