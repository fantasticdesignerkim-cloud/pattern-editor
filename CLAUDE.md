...
**분산**하려면 **부분 드래그**(예산을 나눠 씀). 예산 소진 이동은 "가슴다트를
복제합니다 — 다른 조각/위치 선택" 힌트로 거부.

**남은 것**: (1) 뒤판 예산 게이트는 공용 경로라 함께 타지만 뒤판 검증 아직.
(2) 더 매끄러운 UX로 findMaxSafeAngle에 budget 제약을 녹여 드래그 중 실시간으로
예산 한계에서 멈추게 할 수 있음(현재는 사후 거부) — 필요 시 다음 작업.
