...
**앞으로 할 일이 많다**(바지·코트·자켓·원피스·스커트 — 교재 4권 더). 한 세션이 비싸면 그만큼
못 간다. 아래는 **실측 근거가 있는 규칙**이다.

### 측정된 것 (2026-09)
- 세션 컨텍스트에서 **가장 큰 단일 메시지가 CLAUDE.md 주입**이었다(1MB+ ×여러 번).
  그래서 이 파일을 231,664자 → 14,664자로 줄였다. **세션당 약 7만 토큰의 고정비가 사라졌다.**
  → **완료 기록을 이 파일로 되돌리지 않는다.** 새 기록은 `docs/history/`, 현재 상태는
  `docs/STATUS.md`.

### 지킬 것
1. **큰 tool 결과를 컨텍스트에 끌어들이지 않는다.** 브라우저·스크립트는 **요약해서** 돌려준다.
   (실패 사례: 앞·뒤판 geometry 를 통째로 덤프해 읽었는데, 정작 필요한 건 apex 좌표와 edge
   목록뿐이었다 — 1만 토큰 넘게 썼고 200 토큰이면 됐다.)
2. **파일을 통째로 읽지 않는다.** `grep -n` 으로 위치를 잡고 `sed -n 'a,bp'` 로 그 구간만.
3. **스크린샷은 수치 검증이 끝난 뒤 최종 1장.** 이미지는 비싸다. "됐는지"는 숫자로 먼저 본다.
4. **교재 이미지는 한 번만 연다.** 쪽당 노트를 `docs/book/PNNN.md` 에 남기고, 다음엔 노트를
   읽는다. 수치 하나가 필요하면 전체 쪽이 아니라 `bookpage.py crop` 을 쓴다.
5. **커밋 단위로 세션을 끊는다.** 컨텍스트가 길수록 매 턴이 비싸진다. CLAUDE.md 가 가벼워져
   **새 세션 시작 비용이 싸졌으므로 자주 끊는 쪽이 이제 더 유리하다.**
6. **탐색적 설계를 코드로 더듬지 않는다.** 대안 비교·스펙 작성은 값싼 쪽(ChatGPT)에서 굽고,
   여기서는 **확정된 스펙의 실행·측정·검증**을 한다. 그게 이 도구가 유일하게 할 수 있는 일이다.
   (실패 사례: 플레어 프로토타입을 픽스처가 나빠 두 번 만들었다.)
7. **같은 작업은 레시피를 따른다** — [docs/RECIPE.md](docs/RECIPE.md). 매번 다시 설계하는 데
   토큰이 가장 많이 든다.
