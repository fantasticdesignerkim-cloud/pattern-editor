...
| # | 제목 | 프로덕션 | 골든 |
|---|---|---|---|
| **C0** | `Add regression fixtures for non-monotonic safe intervals` | 무변경 | 신규 |
| C1 | `Extract endpoint evaluation for dart moves` | ① + 이중 게이트 + 일치 assertion | 무변경 |
| C2 | `Extract physical sweep limit from safe angle search` | ② | 무변경 |
| C3 | `Return applicable angle intervals` (+성능 실측) | ③ | 무변경 |
...
**C0이 필수인 이유** — 아래 3개가 아직 픽스처로 없다. 안전망 없이 ③을 건드리면 안 된다:
**0.752°/18.06° 비단조 사례**(③의 가장 중요한 검증), **무관 notch 변화 0**,
**양쪽 부호 비교**. + `perfBaseline.js`(vm 인스트루먼트 상시화).
...
- **(다음 작업) 형상 엔진 재설계 — 순서 ③** — 위 "형상 엔진 재설계 스펙" 섹션 참고.
  ①(회귀 고정)·②(구역 분리 + `prepareDartMoveCandidate` 공유)는 완료. **다음은
  `evaluateMove` 4계층으로 모든 게이트 통합**: `evaluateEndpoint` / `findPhysicalSweepLimit`
  / `findApplicableIntervals`(비단조 안전구간을 **구간 목록**으로) / `resolveRequestedAngle`.
  `prepareDartMoveCandidate`의 `limits`가 이 4계층의 전신이고, gen-0 경로에서 null인
  구간들이 `chooseSignedBaseAngle` 해체 시 채워질 자리다. 착수 승인 대기.
