...
- **(다음 작업) 형상 엔진 재설계 — 순서 ③의 C5** — 위 "형상 엔진 재설계 스펙" 참고.
  **C0·C1·C2·C3·C4 완료.** ①(`evaluateEndpoint`)·②(`findPhysicalSweepLimit`)가 프로덕션
  경로에 배선됐고, 부호 선택은 `findMaxApplicableMagnitude` 위→아래 탐색으로 통합됐다
  (`prepareDartMoveCandidate`가 `selectRotationSign` 하나를 부른다).
  **다음은 C5 `Resolve requested drag angle against applicable intervals`** — ④ +
  `[0, baseAngle]` clamp 교체. **착수 승인 대기.**
  **C5에서 반드시 결정할 것**:
  1. **③ `findApplicableIntervals`의 운명** — C4는 이걸 안 쓴다(탐색으로 충분했다).
     C5가 요청각을 "구간 목록에 맞추는" 방식이면 살리고, **lazy exact-evaluation**
     (요청각을 바로 ①로 평가 → invalid면 가까운 경계만 탐색)이면 테스트/진단 전용으로
     둘지 삭제할지 **명시적으로 결정한다. 미사용인 채 C7 이후까지 남기지 않는다.**
  2. **0° 보존**: `requested === 0` 또는 `|requested| < MIN_DART_ANGLE_RAD` → `resolved = 0`
     (중립 preview). MIN 적용은 ④의 책임이다 — ②/③/C4는 일부러 안 자른다.
  3. **금지구간 스냅**: 가장 가까운 경계 / 드래그 방향 이력 무시 / 동률이면 작은 각도
     (히스테리시스 금지 — preview 재현성).
  4. **최종 차단은 정확한 요청각의 `evaluateEndpoint`**가 한다(격자는 스텝보다 좁은
     금지구간을 보장 탐지 못 함 — **C7에서도 제거 금지**).
  5. **성능**: bake는 이제 병목이 아니다 — **②의 rotColl 60스텝 스캔이 지배**한다.
     `≤1.2×`를 넘으면 스텝을 줄이지 말고 원인을 실측해 보고한다. perf 골든은 shape
     골든과 규칙이 다르다(위 "골든 JSON 변경" 항목).
- **(C5/C7) legacy 순수 삭제 커밋** — `chooseSignedBaseAngle`·`budgetMaxAngle`·
  `applyTimeSafeAngle`·`findMaxSafeAngle`(wrapper)은 C4가 우회했지만 아직 남아있다.
  실제 호출이 0이 된 걸 확인한 뒤 **별도 순수 삭제 커밋**으로 지운다. 그때
  `signSelectionMigration.js`의 legacy 복제도 함께 사라진다(그 파일의 목적이 끝난다).
  위 "Dead code 감사"의 `calc*CloseAngleByRotateHit` 3종도 같이 처리 대상.
